...
bool Gl_widget::init_yuy2_shader()
{
	...
	bool status = yuy2_filter.addShaderFromSourceCode(QGLShader::Vertex,
		"#version 130\n"
        "in vec4 inVertex;\n"
        "in vec2 inTexCoord;\n"
        "out vec4 texCoord;\n"
	...
	status = status && yuy2_filter.addShaderFromSourceCode(QGLShader::Fragment,
		"#version 130\n"
        "in vec4 texCoord;\n"
        "out vec4 fragColor;\n"
...
void Gl_widget::init_yuv_textures()
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE_ALPHA, video_width, video_height, 0, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, 0);
    ...
    glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexEnvf(GL_TEXTURE_ENV,GL_TEXTURE_ENV_MODE,GL_DECAL);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, (video_width>>1), video_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0);
...
void Gl_widget::render_to_texture()
{
    ...
    edge_filter.bind();
    edge_filter.setUniformValue("texture_0", 2);
    ...
    nmes_filter.bind();
    nmes_filter.setUniformValue("texture_0", 3);
    ...
    binary_filter.bind();
    binary_filter.setUniformValue("texture_0", 4);
    binary_filter.setUniformValue("min", (GLfloat)(35.0/255.0));
    binary_filter.setUniformValue("max", (GLfloat)(100.0/255.0));
